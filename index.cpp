...
const int drivePin1 = 10;
const int drivePin2 = 11;
...
// Helper functions for steer and drive abstraction
void drive(int drivePin, int power) {
  analogWrite(drivePin, power);
}

void steer(Servo &servo, int offsetAngle) {
  servo.write(90 + offsetAngle);
}
...
void loop() {
  ...
  if (seeLine3) {
    ...
    drive(drivePin1, 200);
    if (seeLine1) { steer(steering, -40); return; }
    if (seeLine2) { steer(steering, -20); return; }
    if (seeLine4) { steer(steering, 20); return; }
    if (seeLine5) { steer(steering, 40); return; }
  }
}